...
class TGBot{
    ...
    private function Request($link, $data = []){ //thanks to @windoz for helping me to speedup curl
        curl_setopt_array($this->curl, [
    ...
    public function sendMessage($chat_id, $text, $reply_markup = false, $button_type = 'inline', $reply_to_message_id = NULL, $parse_mode = NULL, $disable_web_page_preview = NULL){
        ...
        if($reply_markup){
            if($button_type == 'inline'){
                ...
                $reply_markup = json_encode(array('keyboard' => $reply_markup,'resize_keyboard' => true));
                $args['reply_markup'] = $reply_markup;
            }
        }
